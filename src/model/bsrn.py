...
import math
import model.Upsamplers as Upsamplers
...
def stdv_channels(F):
    ...
    F_variance = (F - F_mean).pow(2).sum(4, keepdim=True).sum(3, keepdim=True).sum(2, keepdim=True) / (F.size(2) * F.size(3)* F.size(4))
...
class BSRN(nn.Module):
    def __init__(self, args, conv='BSConvU', upsampler='pixelshuffledirect', p=0.25):
        super(BSRN, self).__init__()

        num_in_ch = args.n_colors  # 3
        num_out_ch = args.n_colors # 3
        upscale = args.scale[0]  # 4
        num_feat = args.n_feats  # 64
        num_block = args.n_resblocks # 8
        ...
        self.c1 = nn.Conv3d(num_feat * num_block, num_feat, 1)
        self.GELU = nn.GELU()
        ...
        if upsampler == 'pixelshuffledirect':
            ...
            upsampler_module = [common.Upsampler(common.default_conv, upscale, num_feat, act=False), \
                                common.default_conv(num_feat, args.n_colors, 3)]
            self.upsampler = nn.Sequential(*upsampler_module)
