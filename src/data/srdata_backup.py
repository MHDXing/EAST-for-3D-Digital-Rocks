...
class SRData(data.Dataset):
    ...
    def _load_file(self, idx):
        ...
        if self.args.ext == 'img' or self.benchmark:
            ...
            # hr_height, lr_height = hr.shape[0], lr.shape[0]
            # # 使用广播创建具有相同高度的三维数组，第三维度与第一维度大小相同
            # hr = np.tile(hr[np.newaxis, :, :], (hr_height, 1, 1))
            # lr = np.tile(lr[np.newaxis, :, :], (lr_height, 1, 1))  
            hr_height, lr_height = hr.shape[0], lr.shape[0]
            # 使用广播创建具有相同高度的三维数组，第三维度与第一维度大小相同
            hr = np.tile(hr[np.newaxis, :, :], (hr_height, 1, 1))
            lr = np.tile(lr[np.newaxis, :, :], (lr_height, 1, 1))  
    ...
    def get_patch(self, lr, hr):
        ...
        if self.train:
            ...
            if self.args.noise: lr = common.add_noise(lr)
            # print(lr[0])
